...
{
    GJProperty *propertyObj = objc_getAssociatedObject(self, property);
    if (propertyObj == nil) {
...
- (void)setProperty:(objc_property_t)property
{
    ...
    GJJsonAssertParamNotNil(property);
    ...
    _type = [GJPropertyType cachedTypeWithCode:code];
}
...
- (NSArray *)propertyKeysWithStringKey:(NSString *)stringKey
{
    ...
    for (NSString *oldKey in oldKeys) {
        ...
        if (start != NSNotFound) { // 有索引的key
            ...
            if (prefixKey.length) {
                GJPropertyKey *propertyKey = [[GJPropertyKey alloc] init];
            ...
            for (NSInteger i = 0; i<cmps.count - 1; i++) {
                GJPropertyKey *subPropertyKey = [[GJPropertyKey alloc] init];
                subPropertyKey.type = GJPropertyKeyTypeArray;
...
- (void)setPropertyKeys:(NSArray *)propertyKeys forClass:(Class)c
{
    ...
    GJ_LOCK(self.propertyKeysLock);
    self.propertyKeysDict[key] = propertyKeys;
    GJ_UNLOCK(self.propertyKeysLock);
}
...
- (void)setObjectClassInArray:(Class)objectClass forClass:(Class)c
{
    ...
    GJ_LOCK(self.objectClassInArrayLock);
    self.objectClassInArrayDict[key] = objectClass;
    GJ_UNLOCK(self.objectClassInArrayLock);
}
